...
main_menu_font = load_font('Flame.ttf', 32)
big_ass_font = load_font('Flame.ttf', 96)
main_menu_sprite = pygame.image.load(os.path.join('sprites', "flame-menuselect.png"))
...
class MenuEntry():
   ...
   def render_images(self):
      ...
      self.normalimage = main_menu_font.render(self.text, True, (255, 255, 255))
...
class MenuScreen(screen.Screen):
   def __init__(self, menu_entries, vertical_start = 200, title="", select_sound=None, background_surface = None, menu_bg_surface = None):
   ...
   def input(self, events):
      ...
      for event in events :
         if event.type == KEYUP and event.key == K_ESCAPE:
            ...
            # Play menu select sound if applicable
            ...
            for entry in self.entries:
               if(entry.rect.collidepoint(event.pos)) and self.active_index != count:
   ...
   def update(self, screensurface):
      ...
      if len(self.title) > 0:
         if not self.title_image:
            self.title_image = big_ass_font.render(self.title, True, (255,255,255))
